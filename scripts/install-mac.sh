...
if [ "$MACOS_MAJOR" -lt 12 ] 2>/dev/null; then
  die "Radius needs macOS 12 (Monterey) or newer. You are on $(sw_vers -productVersion)."
fi

printf '\n%sRadius%s — installing for macOS %s (%s)\n\n' "$BOLD" "$RESET" "$(sw_vers -productVersion)" "$ARCH"

WORK_DIR="$(mktemp -d -t radius-install)"
...
try_download() {
  ...
  curl -fsSL --max-time 600 -o "$WORK_DIR/radius.zip" "$url" || return 1
...
ensure_node() {
  ...
  curl -fsSL --max-time 600 -o "$WORK_DIR/$tarball" "$url" \
...
fetch_one_branch() {
  ...
  if curl -fsSL --max-time 600 -o "$WORK_DIR/src.tar.gz" \
...
try_build() {
  ...
  if [ -f package-lock.json ]; then
    npm ci --no-audit --no-fund >/dev/null 2>&1 || npm install --no-audit --no-fund >/dev/null 2>&1 \
      || die "Dependency install failed. Run 'npm install' in a clone to see why."
  else
  ...
  info "Compiling…"
  npx electron-vite build >/dev/null 2>&1 || die "The build step failed."

  info "Packaging the app…"
  npx electron-builder --mac dir "--${ARCH}" --publish never >/dev/null 2>&1 \
    || die "Packaging failed. Run 'npm run package:mac' in a clone to see why."

  BUILT_APP="$(find release -maxdepth 2 -name "$APP_NAME" -print -quit)"
  [ -n "$BUILT_APP" ] || die "The build finished but produced no $APP_NAME."
  BUILT_APP="$WORK_DIR/src/$BUILT_APP"
}
...
install_app() {
  ...
  if [ -e "$target" ]; then
    ...
    $sudo_prefix rm -rf "$target" || die "Could not remove the old $APP_NAME. Is Radius running?"
  fi

  $sudo_prefix cp -R "$BUILT_APP" "$target" || die "Could not copy Radius into $INSTALL_DIR."
  $sudo_prefix xattr -dr com.apple.quarantine "$target" 2>/dev/null || true

  printf '\n%s✓%s Radius is installed at %s\n\n' "$GREEN" "$RESET" "$target"
}
